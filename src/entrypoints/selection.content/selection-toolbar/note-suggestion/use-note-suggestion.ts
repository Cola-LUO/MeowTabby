 ...
 * pre-request gate is hosted status: a Built-in provider whose billing
 * verdict is explicitly unavailable (signed out) skips without issuing the
 * doomed stream call. A valid
  ...
  const maybeFire = useCallback((input: NoteSuggestionFireInput) => {
    ...
    const run = async () => {
      ...
      if (provider.kind === "system") {
        // A background auto-fire feature must not hammer a hosted account it
        // cannot use: skip silently on the explicit unavailable verdict —
        // signed out — since nobody is watching to act on the error and
        // status is far cheaper than a doomed stream call. Balance is not
        // this gate's question (the backend answers an out-of-balance
        // /v1/generate itself, 402). Fail open when there is no verdict —
        // the run surfaces any real error, which `fetchHostedAiStatus`
        // already does; going through it also shares the request with
        // whatever else resolves a hosted ref at the same moment, since one
        // status response covers every feature.
        const status = await fetchHostedAiStatus()
        if (signal.aborted) {
          return
        }
        if (status && !status.authenticated) {
          logger.info("[NoteSuggestion] Skipped: hosted account not signed in")
