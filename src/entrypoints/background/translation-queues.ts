...
export async function createTranslationQueues<TContext>(
  config: TranslationQueueSetupConfig<TContext>,
) {
  ...
  // Two lanes per channel: the user-configured rate applies only to
  // local/BYOK requests; hosted (billing) requests are pinned to rate 1 /
  // capacity 1 to match the backend's one-in-flight-generate limit (§6.4).
  ...
  const batchQueue = new BatchQueue<TranslateBatchData<TContext>, string>({
    ...
    // Narrow port, not the whole queue: while either rate limiter has no free
...
function watchQueueConfig(
  ...
  queuesPromise: Promise<{
    localRequestQueue: RequestQueue
    hostedRequestQueue: RequestQueue
  ...
  storage.watch<Config>(`local:${CONFIG_STORAGE_KEY}`, (newConfig) => {
    if (!newConfig) return
    // Only the local/BYOK queue follows the user's rate config. The hosted
    // queue is pinned to rate 1 (billing in-flight = 1) and must not drift.
...
export function setUpWebPageTranslationQueue(): void {
  ...
  onMessage("enqueueTranslateRequest", async (message) => {
    const { localRequestQueue, batchQueue } = await queuesPromise
    const {
      data: {
        text,
        langConfig,
    ...
    if (shouldUseBatchQueue(providerRef)) {
      ...
      result = await localRequestQueue.enqueue(thunk, scheduleAt, hash, scope ? [scope] : undefined)
    }
...
export function setUpSubtitlesTranslationQueue(): void {
  ...
  onMessage("enqueueSubtitlesTranslateRequest", async (message) => {
    ...
    if (shouldUseBatchQueue(providerRef)) {
      ...
      result = await localRequestQueue.enqueue(thunk, scheduleAt, hash)
    }
