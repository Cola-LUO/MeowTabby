...
describe("background-stream", () => {
  ...
  // mid-stream SSE failure propagates through `consumeTextPartStream`. Both
  // must reach the RequestQueue with their billing retry meta intact — either
  // one coming back retryable makes the queue burn its whole backoff budget
  // on a pricing wall that never moves.
  it("drains the backlog on billing access denials from a hosted text stream", async () => {
    const denial = () =>
    ...
    const expectQueueFatal = (caught: unknown) => {
      ...
      expect(caught).toMatchObject({ statusCode: 401, kind: "access-denied" })
    ...
    createBillingTextPartStreamMock.mockRejectedValueOnce(denial())
    expectQueueFatal(await runAndCatch())

    // Refused after the first part — a mid-stream SSE failure.
    createBillingTextPartStreamMock.mockResolvedValueOnce(
      (async function* () {
  ...
  it("rejects hosted text streams without a requestId as invalid_request", async () => {
    const { runStreamTextInBackground } = await import("../background-stream")
    ...
    expect(caught).toBeInstanceOf(Error)
    expect((caught as Error & { code?: string }).code).toBe("invalid_request")
    expect(createBillingTextPartStreamMock).not.toHaveBeenCalled()
  })

  it("defaults the hosted billing feature to pageTranslation when hostedFeature is absent", async () => {
    createBillingTextPartStreamMock.mockResolvedValue(
      (async function* () {
    ...
    expect(createBillingTextPartStreamMock).toHaveBeenCalledWith(
      expect.objectContaining({ feature: "pageTranslation" }),
      undefined,
    )
    expect(getModelByIdMock).not.toHaveBeenCalled()
  ...
  it("labels hosted selectionTranslation text streams with the route's billing feature", async () => {
    createBillingTextPartStreamMock.mockResolvedValue(
      (async function* () {
        yield { type: "text-delta", text: "Hola" }
    ...
    // Exact wire payload: hostedFeature becomes the billing feature label and
    // never rides along into the billing request body under its own name.
    expect(createBillingTextPartStreamMock).toHaveBeenCalledWith(
      {
  ...
  it("overrides the billing feature label with billingTag so summaries bill as summarization", async () => {
    createBillingTextPartStreamMock.mockResolvedValue(
      (async function* () {
    ...
    expect(createBillingTextPartStreamMock).toHaveBeenCalledWith(
      expect.objectContaining({ feature: "summarization" }),
      undefined,
    )
  })
